...
async function capture(){
 ...
 try{
 for(const name of (claimOnly?['demo-claim-receipt']:['demo-receipt','demo-claim-receipt'])){
 const receiptPage=await browser.newPage({viewport:{width:700,height:1000}});
 await receiptPage.goto(pathToFileURL(path.join(__dirname,name+'.html')).href);await receiptPage.evaluate(()=>document.fonts.ready);await receiptPage.screenshot({path:path.join(__dirname,name+'.png')});await receiptPage.close();
 }
 ...
 await page.route('**/*',async route=>{
  ...
  if(u.pathname==='/store-demo-claim-receipt.png')return route.fulfill({path:path.join(__dirname,'demo-claim-receipt.png'),contentType:'image/png'});
 ...
 await page.getByText('공지',{exact:true}).click();await page.getByText('10월 정기산행 안내',{exact:true}).click();await page.getByText('읽음 6 / 8명',{exact:true}).waitFor();await page.getByText('알림 보냄',{exact:true}).first().waitFor();await close();
 await page.getByText('8월 결산',{exact:true}).click();await page.getByText('결산 보기',{exact:true}).click();await page.getByText('당월 이월',{exact:true}).waitFor();await snap('09-closing');await close();await close();
 await page.getByRole('tab',{name:'장부',exact:true}).click();await page.getByText('예산',{exact:true}).click();await page.getByText('작년 1,680,000 · 가져온 값',{exact:true}).waitFor();await snap('08-budget');
 await page.getByRole('tab',{name:'설정',exact:true}).click();await page.getByText('모임 정보 · 월 회비',{exact:true}).click();
 chooser=page.waitForEvent('filechooser');await page.getByText('폰에 있는 파일 고르기',{exact:true}).click();await(await chooser).setFiles({name:'2025년_등산모임_장부.csv',mimeType:'text/csv',buffer:Buffer.from('날짜,구분,항목,내용,금액\n2025-12-07,수입,회비,12월 회비 9명,270000\n2025-12-14,지출,식비,송년 산행 식사,84000\n2025-12-14,지출,교통비,주차비 3대,36000\n2025-12-14,지출,준비물,생수와 간식,18000')});await page.getByText('기록 4건을 찾았어요',{exact:true}).waitFor();await snap('07-import');
 }
 // Second fictional login: member 이서연. Separate supplies receipt through the actual upload flow, no app source edits.
 state={uploaded:false,member:true};await page.addInitScript(()=>localStorage.setItem('cm.member',JSON.stringify({id:902,name:'이서연',provider:'google',needsSignup:false})));await page.goto(base,{waitUntil:'networkidle'});
 chooser=page.waitForEvent('filechooser');await page.getByLabel('영수증 찍기',{exact:true}).click();await(await chooser).setFiles(path.join(__dirname,'demo-claim-receipt.png'));await page.getByText('1장 전송하기',{exact:true}).click();await page.getByText('지금 기록',{exact:true}).click();await page.getByText('지금 기록',{exact:true}).waitFor({state:'hidden'});await page.getByText('받을 계좌',{exact:true}).waitFor();await snap('06-claim');
