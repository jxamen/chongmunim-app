...
`src/loginRescue.ts` 를 그 함수로 바꾼다 — **이 건만으로 새 빌드를 굽지 말고** 다음 OTA 에 얹는다.
2.2.0 은 **순수 JS**(네이티브 변경 없음 — 문의요청 확인, jcurve-packages da46a00 · 0b6e96a)라 네이티브 빌드를 기다릴 필요가 없다.
**막힌 것**: 2026-09-23 현재 GitHub **릴리스 자산이 없다**(태그는 auth-v2.1.3 까지, 2.2.0 tgz 는 404) — `package.json` 의 릴리스 주소로
설치가 안 되니 올라온 뒤에 바꾼다. 지금은 `loginRescue.ts` 로 이미 고쳐져 있어(테스트 4개) 급하지 않다.
